...
const Util = require("canvacord").Util;
const assets = require("canvacord").Assets;

/**
 * Nowplaying presence card builder
 */
class Nowplaying {

    /**
     * Creates Nowplaying presence card
     * @example
     * const card = new canvacord.Nowplaying()
        .setAuthor("Indila")
        .setAlbum("Mini World")
    ...
    card.build()
        .then(data => {
            canvacord.write(data, "./images/Nowplaying.png");
        });
     */
    constructor() {
        /**
        ...
        /**
         * Image Format (false == 1:1 | true == 16:9)
         * @type {string}
         */
        ...
        /**
         * @typedef {object} NowplayingDataBG
        ...
        /**
         * @typedef {object} NowplayingProgressBarData
        ...
        /**
         * Progressbar details
         * @type {NowplayingProgressBarData}
         */
        this.progressBar = {
            bgColor: "#E8E8E8",
            color: "#1DB954"
    ...
    setFormat(input) {
        if (!input || typeof input !== "boolean") throw new Error(`Unxpected Boolean, received ${typeof title}!`);
        this.imgformat = input;
        return this;
    }
    /**
     * Set image
    ...
    /**
     * This function converts raw data into Nowplaying presence card.
    ...
    async build() {
        ...
        const progressbarcolor = this.progressbarcolor || "#1DB954";
        ...
        // draw image
        if(this.imgformat == true){
            const img = await Canvas.loadImage(this.image);
            ctx.drawImage(img, 15, 34.5, 144, 81);
        }else {
            const img = await Canvas.loadImage(this.image);
            ctx.drawImage(img, 15, 15, 120, 120);
        }
        ...
        await Util.renderEmoji(ctx, Util.shorten(this.title, 40), 170, 40);
        ...
        ctx.font = "14px Manrope";
        await Util.renderEmoji(ctx, ending, 530, 130);
        ...
        // progressbar track
        ctx.rect(170, 170, 400, 4);
        ctx.fillStyle = this.progressBar.bgColor
        ctx.fillRect(170, 110, 400, 4);

        // progressbar
        ctx.fillStyle = this.progressBar.color;
        ctx.fillRect(170, 110, this.__calculateProgress(progress, total), 5);
    ...
    __calculateProgress(progress, total) {
        let prg = (progress / total) * 400;
        console.log(prg)
        if (isNaN(prg) || prg < 0) return 0;
        if (prg > 400) return 400;
        return prg;
    }
